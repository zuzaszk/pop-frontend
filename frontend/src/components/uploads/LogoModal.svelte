<script>
  ...
  async function onUpload(event) {
    ...
    try {
      const response = await fetch(
        `http://192.168.0.107:8080/zpi/projectElements/uploadElement`,
        {
          method: "POST",
          body: formData,
        }
  ...
  onMount(async () => {
    try {
      const response = await fetch(
        `http://192.168.0.107:8080/zpi/deadlines/getDeadlineByProjectIdAndElementTypeId?projectId=${projectId}&elementTypeId=${elementTypeId}`
      );
