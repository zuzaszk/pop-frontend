  ...
  async function submitComment() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        submitCommentErrorMessage = "Failed to submit the comment.";
      }
    } catch (err) {
      submitCommentErrorMessage = `Error: ${err.message}`;
    } finally {
    ...
    <div class="p-4">
      <div class="flex flex-col lg:flex-row justify-between">
        <div class="lg:w-2/3">
          ...
          <h2 class="text-xl font-bold text-[#2C3E50] mb-2">Description</h2>
          <p class="text-lg text-[#7F8C8D] mb-6">
            {project.description || "No overview available."}
          </p>
          <h3 class="text-lg font-bold text-[#2C3E50]">Language:</h3>
          <p class="text-[#7F8C8D] font-medium mb-6">
            {getLanguageName(project.language)}
  ...
  async function submitComment() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const result = await response.json();
        submitCommentErrorMessage = result.message || "Failed to submit the comment.";
      }
    } catch (err) {
      submitCommentErrorMessage = `Error: ${err.message}`;
    } finally {
