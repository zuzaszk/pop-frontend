...
<script>
  ...
  async function fetchComments(elementId) {
    try {
      const response = await fetch(
        `http://localhost:8080/zpi/comments/getByElementId?elementId=${elementId}`
      );
      if (response.ok) {
  ...
  async function fetchProjectDetails() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (posterElement) {
          posterUrl = `http://localhost:8080/zpi/projectElements/retrieve?projectElementId=${posterElement.elementId}`;
        }
      } else {
        error = "Failed to load project details.";
      }
