<!-- <script>
    ...
    async function fetchProjectDetails() {
        ...
        try {
            const response = await fetch(`http://localhost:8080/zpi/project/basicInfo?projectId=${projectId}`);
            if (response.ok) {
    ...
    try {
        const response = await fetch('http://localhost:8080/zpi/project/saveBasicInfo', {
            method: 'PUT',  
...
<div class="container bg-white p-6 rounded-lg shadow-md mx-auto overflow-auto">
  <h1 class="text-3xl font-bold text-[#2C3E50] text-center mb-1">{title}</h1>
  <h2 class="text-lg text-gray-500 text-center mb-6">Edition: {edition}</h2>
