...
<script>
  // Import modal components for different upload types
  import LogoModal from "../../components/uploads/LogoModal.svelte";
  import SourceCodeModal from "../../components/uploads/SourceCodeModal.svelte";
  import PremiseModal from "../../components/uploads/PremiseModal.svelte";
  import PosterModal from "../../components/uploads/PosterModal.svelte";
  ...
  // Boolean variables to track upload status of each type
  let isLogoUploaded = false;
  let isSourceCodeUploaded = false;
  let isPremiseUploaded = false;
  let isPosterUploaded = false;
  ...
  // Boolean variables to track modal visibility for each type
  let showLogoModal = false;
  let showSourceCodeModal = false;
  let showPremiseModal = false;
  let showPosterModal = false;
  ...
  // Function to toggle the upload status based on the file type
  function toggleUploadStatus(type) {
    switch (type) {
      case "logo":
        isLogoUploaded = !isLogoUploaded;
...
<div class="page-container">
  <div class="container mx-auto bg-white p-6 rounded-lg shadow-lg">
    <!-- Page title -->
    <h2 class="title text-3xl font-bold text-[#2C3E50] mb-8 text-center">
      PoP Project Upload Center
    </h2>

    <!-- Description paragraph -->
    <p class="text-gray-600 mb-4 text-center">
      Upload your project materials to ensure transparency and organized management for evaluations.
    </p>

    <!-- Upload grid containing file upload options -->
    <div class="upload-grid">
      <!-- Logo Upload Button -->
      <button
        on:click={() => (showLogoModal = true)}
...
<style>
  ...
  /* Grid layout for upload buttons */
  .upload-grid {
    display: grid;
    grid-template-columns: repeat(2, 1fr);
    gap: 1.5rem;
  ...
  /* Styling for individual upload items */
  .upload-item {
    display: flex;
    flex-direction: column;
    align-items: center;
  ...
  /* Hover effect for upload items */
  .upload-item:hover {
    transform: scale(1.05);
    border-color: #3498db;
  }

  /* Styling for uploaded status */
  .status-added {
    border-color: #2ecc71;
  }
