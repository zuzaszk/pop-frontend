<!-- <script>
    ...
    onMount(async () => {
      try {
        const response = await fetch('http://192.168.0.107:8080/zpi/editions/listAll');
        if (response.ok) {
    ...
    async function createProject() {
      ...
      try {
        const response = await fetch(
          `http://192.168.0.107:8080/zpi/project/create?title=${encodeURIComponent(projectName)}&editionId=${selectedEdition}`,
          {
            method: 'POST',
          }
        );
...
<script>
  ...
  onMount(async () => {
    try {
      const response = await fetch(
        "http://192.168.0.107:8080/zpi/editions/listAll"
      );
      if (response.ok) {
        editions = await response.json();
      } else {
